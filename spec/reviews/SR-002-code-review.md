...
| FND-215 | medium | Resolved: every local-CI prerequisite has a behavioral failure probe; Make ignore modes and tool overrides are rejected; the collector sanitizes gate-control variables; retained output contradicting a zero exit status fails re-derivation; direct and conditional ignored Rust tests are rejected; trace references/statuses/diagnostics are checked; evidence membership, symlinks, records, and outcome statuses are censused; and required JSON formats fail closed. | FR-004, FR-005, NFR-001, NFR-002, TC-017, TC-022 |
