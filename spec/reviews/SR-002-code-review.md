...
| FND-215 | medium | Resolved after follow-up: checked recipes use fixed tool names; local CI refuses all ambient Make controls, tool overrides, and optimized Python; static, expanded-recipe, and per-command probes reject false-success controls at every recipe position. The collector applies one sanitized environment to every retained command, and finalization requires a positive Rust/corpus output census in addition to rejecting contradictions. Direct and conditional ignored Rust tests, trace references/statuses/diagnostics, evidence membership/symlinks/records, and required JSON formats are all behavior-tested. | FR-004, FR-005, NFR-001, NFR-002, TC-017, TC-022 |
...
| FND-217 | high | Resolved: exact-candidate evidence must positively prove eight Rust test-result groups and the complete corpus checksum census; the assurance-bound record must have every outcome passed. The parameter digest and envelope result are re-derived from the named source revision, and active-collection markers require a live collector ancestor rather than a caller-supplied token. | FR-005, NFR-002, TC-022 |
