...
| FND-217 | high | Resolved: exact-candidate evidence matches Cargo's compiled tests to the exact requirement-tagged source census plus the complete local-gate transcript census; the assurance-bound record must have every outcome passed and cover the current non-evidence tree. Parameter digests, tool identities, and envelope results are re-derived from the named source revision. Record-history checks are explicitly relative to the presented Git history; remote branch protection and review remain the non-local control. | FR-005, NFR-002, NFR-003, TC-022, TC-023, TC-024, TC-025 |
| FND-218 | high | Resolved during final QA: `make ci` delegates to one transcript runner which executes the independent clean-tree retained-evidence verification first and exactly once, so an uncommitted early return cannot become a single-file success key. | NFR-003, TC-024 |
| FND-219 | medium | Resolved during final QA: the collector obtains locked tool fields through the `tool_identity.py` CLI, which is executable under the exact `env -i` collection environment; all nine path and digest lookups have a clean-environment behavior test. | NFR-003, TC-023 |
