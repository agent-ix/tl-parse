...
| FND-217 | high | Resolved: exact-candidate evidence must positively prove at least 25 passing Rust tests across eight result groups plus the complete corpus, policy, traceability, attribution, and sanitizer census; the assurance-bound record must have every outcome passed and cover the current non-evidence tree. Parameter digests and envelope results are re-derived from every named source revision, while collection stages outside the evidence tree and record history is append-only under Git review. | FR-005, NFR-002, TC-022 |
