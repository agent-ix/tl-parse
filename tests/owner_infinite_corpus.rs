...
#[test]
fn pinned_owner_corpus_formats_and_reparses_through_v4() {
    ...
    for case in cases["cases"].as_array().unwrap() {
        ...
        let text = first
            .text
            .unwrap_or_else(|| panic!("owner {id}: {:?}", first.error));
        ...
        assert_eq!(
            parsed.fairness.as_ref().map(|f| f.roots().len()),
            fairness.as_ref().map(|f| f.roots().len()),
            "{id}"
        );
        let again =
